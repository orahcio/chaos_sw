...
int main(int argc, char *argv[]) {
  ...
  if(argc<14) {
    printf("Use %s <Dados de entrada:> <12. T mcs> <13. T mcs para salvar>\n",argv[0]);
    ...
    printf("8. <# de amostras>");
    printf("9. <p inicial>\n");
    printf("10. <p final>\n");
    printf("11. <# de p's>\n");
    
    printf("Veja o cabeçalho do fonte para as definições dos parametros\n A saida leva os parametros no nome.\n");
    exit(-1);
  }

  unsigned long int sem, sem_rede;
  int i, A=atoi(argv[8]), **rede, *s, *s0, num_p=atoi(argv[11]);
  double  c_0=strtod(argv[7],NULL), rho=0;
  double p_i=strtod(argv[9],NULL), p_f=strtod(argv[10],NULL), delta_p=(p_f-p_i)/(double)num_p;
  par P;
  char out[255], ai[255];
  gsl_rng *r;

  // Copia os parâmetros da lista de argumetos
  P.T_mcs=atoi(argv[12]); P.D_mcs=atoi(argv[13]);
  n=atoi(argv[1]);
  K=atoi(argv[2]);
  ...
  for(p=p_i;p<p_f;p+=delta_p) {
    makesw(rede,r); // Uma rede para cada p
    for(i=0;i<A;i++) {
      strcpy(ai,out);
